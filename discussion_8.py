...
def getLink(soup):
    ...
    a_list = soup.find('a', title = "List of American universities with Olympic medals")
    return url + a_list.get("href")
...
def getAdmissionsInfo2019(soup):
    d = {}
    get_table = soup.find('table', class_ = "toccolours")
    get_keys = get_table.find_all('tr')
    for item in get_keys[3:]:
        get_values = item.find_all('td')
        key = get_values[0].text.strip()
        value = get_values[1].text.strip()
        d[key] = value
    return d
